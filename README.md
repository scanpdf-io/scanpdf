...
- **Five languages** — English, Spanish, German, Portuguese and French,
  each on its own URL with the interface and the documentation translated.
...
The site ships in English, Spanish, German, Portuguese and French. English
lives at the root; the others live under `/es/`, `/de/`, `/pt/` and `/fr/`,
each with its own localized page slugs and a reciprocal `hreflang` cluster.
...
code to `LANGS` in `tools/build-i18n.py`. The generator refuses to render a
page with a missing key rather than emitting a blank.
